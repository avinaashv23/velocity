...
export const EditProjectModal: React.FC<EditProjectModalProps> = ({ project, onClose }) => {
  const { clients, updateProject } = useApp();

  const [name, setName] = useState(project.name);
  const [description, setDescription] = useState(project.description);
  const [clientId, setClientId] = useState(project.clientId);
  const [dueDate, setDueDate] = useState(project.dueDate);
  const [budget, setBudget] = useState(project.budget || '$50,000');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const res = updateProject(project.id, {
      name: name.trim(),
      ...
      <div className="w-full max-w-lg rounded-2xl bg-[#151c28] border border-[#26354f] p-6 shadow-2xl space-y-5">
        <div className="flex items-center justify-between border-b border-[#232f44] pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-slate-100">Edit Project Settings</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-slate-300 font-semibold mb-1">Health Status</label>
              <select
                value={status}
